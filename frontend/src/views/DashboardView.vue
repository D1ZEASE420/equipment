  ...
  <div class="space-y-6">
    ...
    <div class="rounded-xl bg-gradient-to-r from-primary-600 to-primary-800 p-6 text-white">
      ...
      <p class="mt-1 text-primary-100 text-sm">
        {{ auth.isAdmin ? 'Admin dashboard — manage devices and monitor borrowings.' : 'Browse devices and borrow equipment using your student account.' }}
      </p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      ...
      <StatCard label="Borrowed"       :value="stats.borrowed"  color="amber" >
        <template #icon>
          <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
    ...
    <div class="card">
      <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
        <h3 class="font-semibold text-gray-900">
          {{ auth.isAdmin ? 'Active Borrowings' : 'My Current Borrowings' }}
        </h3>
      ...
      <div v-else class="divide-y divide-gray-50">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="truncate text-sm font-medium text-gray-900">{{ b.device.name }}</p>
            <p class="text-xs text-gray-400 font-mono">{{ b.device.serial_number }}</p>
          </div>
          <div v-if="auth.isAdmin" class="hidden text-sm text-gray-500 sm:block">{{ b.user.name }}</div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <RouterLink to="/borrow" class="card group flex items-center gap-4 p-5 transition-shadow hover:shadow-md">
        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-primary-100 text-primary-600 group-hover:bg-primary-600 group-hover:text-white transition-colors">
          <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div>
          <p class="font-semibold text-gray-900">Scan & Borrow</p>
          <p class="text-sm text-gray-400">Use camera or enter serial</p>
        </div>
      </RouterLink>

      <RouterLink to="/devices" class="card group flex items-center gap-4 p-5 transition-shadow hover:shadow-md">
        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
          <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
        </div>
        <div>
          <p class="font-semibold text-gray-900">Browse Devices</p>
          <p class="text-sm text-gray-400">View all equipment</p>
        </div>
      </RouterLink>

      <RouterLink v-if="auth.isAdmin" to="/admin" class="card group flex items-center gap-4 p-5 transition-shadow hover:shadow-md">
        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-100 text-purple-600 group-hover:bg-purple-600 group-hover:text-white transition-colors">
          <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div>
          ...
          <p class="text-sm text-gray-400">Manage inventory</p>
        </div>
      </RouterLink>
    </div>
  </div>
